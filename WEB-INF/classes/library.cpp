...
JNIEXPORT jboolean JNICALL Java_Login_checkadmin (JNIEnv* env, jobject thisObj, jstring usname, jstring pswd)
{
    try
    {
        ...
        pqxx::result response = worker.exec("SELECT id,email FROM ADMINS WHERE EMAIL='" + us + "' AND PASSWORD = md5('"+ ps +"') ");
...
JNIEXPORT jboolean JNICALL Java_Login_checkuser(JNIEnv* env, jobject thisObj, jstring usname, jstring pswd) {
    try
    {
        ...
        pqxx::result response = worker.exec("SELECT id,email FROM users WHERE email = '" + us + "' AND password = md5('" + ps + "')");
...
JNIEXPORT void JNICALL Java_Borrow_borrowBook(JNIEnv* env, jobject thisObj, jint bid, jstring username)
{
    ...
    try {
        ...
        pqxx::result response = W.exec("SELECT * FROM books");
...
JNIEXPORT void JNICALL Java_Return_returnBook(JNIEnv* env, jobject thisObj, jint bid, jstring username)
{
    int uid;
    ...
    try
    {
        ...
        pqxx::result response = W.exec("SELECT * FROM user_login");
        ...
        W.commit();
        std::cout << "Books Returned successfully" << endl;
    }
    catch (const std::exception& e)
    {
        std::cerr << e.what() << std::endl;
...
JNIEXPORT jstring JNICALL Java_Search_searchBook(JNIEnv* env, jobject thisObj, jstring bname)
{
